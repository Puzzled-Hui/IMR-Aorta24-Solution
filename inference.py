...
## This environment is for nnUNet
os.environ['nnUNet_results'] = "resources"
...
from utils import mkdir, load_itk_image_new, save_itk_new, locate_lungbox
from lungmask_network import LMInferer
...
parser.add_argument('-i', '--input', type=str, required=False, default=INPUT_PATH,
                    help='input folder. Remember to use the correct channel numberings for your files (_0000 etc). '
                         'File endings must be the same as the training dataset!')
parser.add_argument('-o', '--output', type=str, required=False, default=OUTPUT_PATH,
                    help='Output folder. If it does not exist it will be created. Predicted segmentations will '
                         'have the same name as their source images.')
parser.add_argument('-m', type=str, required=False, default=RESOURCE_PATH,
...
## large step-size for fast prediction
# parser.add_argument('-step_size', type=float, required=False, default=0.8,
#                     help='Step size for sliding window prediction. The larger it is the faster but less accurate '
#                          'the prediction. Default: 0.5. Cannot be larger than 1. We recommend the default.')

## disable the tta for the fast prediction
...
def copy_and_rename_files_for_nnUNet_predict(nnUNet_in_dir, crop_info_dir):
    origin_input_folder = args.input
    in_dir = INPUT_PATH / "images/ct-angiography"
    in_filelist = sorted(os.listdir(in_dir))
    # print(in_filelist)
    ...
    ## add the crop folder information
    if os.path.exists(crop_info_dir):
        shutil.rmtree(crop_info_dir)
    mkdir(crop_info_dir)

    for idx, file in enumerate(in_filelist):
        index = "{:03}".format(int(idx + 1))
        image = sitk.ReadImage(os.path.join(in_dir, file))
        out_path = os.path.join(nnUNet_in_dir, 'Aorta24_' + str(index) + '_0000.nii.gz')
        sitk.WriteImage(image, out_path)

        ## add the information here:
        image, origin, spacing, direction = load_itk_image_new(out_path)
        image_shape = image.shape
        ## temporary info here: [0:1] z [0.3:0.75] y [0.25:1] x

        box = np.array([[0, int(image.shape[0] - 1)], [int(image.shape[1] * 0.3), int(image.shape[1] * 0.75)], [
            int(image.shape[2] * 0.25), int(image.shape[2] - 1)]])


        image_crop = image[box[0, 0]:box[0, 1], box[1, 0]:box[1, 1], box[2, 0]:box[2, 1]]
        save_itk_new(image_crop, out_path, origin, spacing, direction)
        box_path = os.path.join(crop_info_dir, 'Aorta24_' + str(index) + '.json')
        with open(box_path, 'w') as file:
            json.dump(box.tolist(), file, indent=4)
...
# Use the lung crop method
def crop_copy_and_rename_files_for_nnUNet_predict(nnUNet_in_dir, crop_info_dir):
    origin_input_folder = args.input
    in_dir = INPUT_PATH / "images/ct-angiography"
    in_filelist = sorted(os.listdir(in_dir))
    ...
    for idx, file in enumerate(in_filelist):
        ...
        image_sitk = sitk.ReadImage(out_path)
        lobe_extractor = LungMaskExtractionModel()
        torch.cuda.empty_cache()
        start_time = time.time()
        with torch.no_grad():
            lobe = lobe_extractor.predict(image_sitk)
        torch.cuda.empty_cache()
        print('the lung lobe extraction taken time:{} s'.format(time.time() - start_time))
        lobe = np.where(lobe > 0, 1, 0)
        lungbox = locate_lungbox(lobe, margin=0)
        lungbox_list = lungbox.tolist()
        z_center_lung = (lungbox[0] + lungbox[1]) / 2
        y_center_lung = (lungbox[2] + lungbox[3]) / 2
        x_center_lung = (lungbox[4] + lungbox[5]) / 2
        xl_ratio = 0.15
        xl_new = lungbox_list[4] + (x_center_lung - lungbox_list[4]) * xl_ratio
        xl_new = int(xl_new)
        xr_ratio = 0.15
        xr_new = lungbox_list[5] - (lungbox_list[5] - x_center_lung) * xr_ratio
        xr_new = int(xr_new)

        box = np.array([[0, int(image.shape[0])], [xl_new, xr_new], [lungbox[2], lungbox[3]]])
        image_crop = image[box[0, 0]:box[0, 1], box[1, 0]:box[1, 1], box[2, 0]:box[2, 1]]
        save_itk_new(image_crop, out_path, origin, spacing, direction)
        box_path = os.path.join(crop_info_dir, 'Aorta24_' + str(index) + '.json')
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            if i == 0:
                ...
                print('================= trainer name is :{} ==============='.format(trainer_name))
    ...
    ## Do not use the more efficient slide window
    ...
    # ## More efficient slide-window strategy ...
    # def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
    #     slicers = []
    #
    #     # x-axis remains the same, x may be the z size
    #
    #     print('the image size[0] is {}'.format(image_size[0]))
    #
    #     x_steps = compute_steps_for_sliding_window((image_size[0],), (self.configuration_manager.patch_size[0],),
    #                                                self.tile_step_size)[0]
    #
    ...
    #         third_patch_start = min(image_size[dim] - self.configuration_manager.patch_size[dim],
    #                                 center_start + self.configuration_manager.patch_size[dim] // 3)
    #
    #         if dim == 1:
    ...
    #         slicers.append(tuple([slice(None), *[slice(si, si + ti) for si, ti in zip((sx, y_steps[1], z_steps[1]),
    ...
    #     if self.verbose:
    #         print(
    #             f'n_steps {len(x_steps) * len(y_steps) * len(z_steps)}, image size is {image_size}, tile_size {self.configuration_manager.patch_size}, '
...
def nnUNet_predict(nnUNet_in_dir, nnUNet_out_dir):
    ...
    print('disable_tta: ', args.disable_tta)
...
class InnerTransform(object):
    def __init__(self):
        self.KeepLargestConnectedComponent = KeepLargestConnectedComponent(applied_labels=[18,19,20,21,22,23],connectivity=3)
...
def largest_connected_component_extraction(img):
    img = InnerTransformer.AddChannel(img)
    img = InnerTransformer.KeepLargestConnectedComponent(img)
    img = InnerTransformer.SqueezeDim(img)
    img = InnerTransformer.CastToNumpyUINT8(img)
    return img


def mapback(nnUNet_out_dir, crop_info_dir):
    pred_filelist = sorted(os.listdir(nnUNet_out_dir))
    in_dir = INPUT_PATH / "images/ct-angiography"
    out_dir = OUTPUT_PATH / "images/aortic-branches"
    mkdir(out_dir)
    original_image_filelist = sorted(os.listdir(in_dir))
    box_info_list = sorted(os.listdir(crop_info_dir))
    # print(original_image_filelist)
    for idx, file in enumerate(pred_filelist):
        original_image = sitk.ReadImage(os.path.join(in_dir, original_image_filelist[idx]))
        original_shape = list(reversed(original_image.GetSize()))
        with open(os.path.join(crop_info_dir, box_info_list[idx]), 'r') as f:
            box = json.load(f)
            box = np.array(box)
            # print(box)
        pred_back = np.zeros(shape=(original_shape[0], original_shape[1], original_shape[2]), dtype=np.uint8)
        pred, origin, spacing, direction = load_itk_image_new(os.path.join(nnUNet_out_dir, file))
        # ## add the largest connected component extraction
        # if idx in lcc_idx:
        #     print('idx:{} use lcc'.format(idx))
        #     pred = largest_connected_component_extraction(pred)

        pred = largest_connected_component_extraction(pred)
        pred_back[box[0, 0]:box[0, 1], box[1, 0]:box[1, 1], box[2, 0]:box[2, 1]] = pred
        original_file = original_image_filelist[idx]
        save_itk_new(pred_back, os.path.join(out_dir, original_file), origin, spacing, direction)


def run():
    nnUNet_in_dir = 'nnUNet_input'
    nnUNet_out_dir = 'nnUNet_output'
    crop_info_dir = 'crop_output'

    ## step 1.  transform the casename for nnUNet prediction usage
    copy_and_rename_files_for_nnUNet_predict(nnUNet_in_dir, crop_info_dir)
    # crop_copy_and_rename_files_for_nnUNet_predict(nnUNet_in_dir, crop_info_dir)
    ## step 2.  use nnUNet to predict results
    start_time = time.time()
    nnUNet_predict(nnUNet_in_dir, nnUNet_out_dir)
    print('Before Optimization Prediction Time Taken:{} s'.format(time.time() - start_time))
    ## step 3.  map back ...
    mapback(nnUNet_out_dir, crop_info_dir)
    print('Saved!!!')
    return 0
